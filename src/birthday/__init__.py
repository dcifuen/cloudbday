...
if env == constants.ENV_DEVELOPMENT:
    ...
    app.config.from_object('birthday.settings.Development')
    ...
    app.config.from_object('birthday.settings.Testing')

elif constants.ENV_PRODUCTION:
    app.config.from_object('birthday.settings.Production')

elif constants.ENV_STAGING:
    app.config.from_object('birthday.settings.Staging')

else:
    logging.error('Unable to identify the environment, using default config')
    app.config.from_object('birthday.settings.Config')
